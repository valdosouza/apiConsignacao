...
class AuditLogStockStatementOrderStockTransfer extends AuditLog {
      ...
      try {
        ...
        if (orderList.length > 0) {
          for (var item of orderList) {
            ...
            //Saida do estoque do Cliente            
            dataItem.tb_stock_list_id = item.tb_stock_list_id_ori;
            dataItem.direction = 'S';
            await StockStatement.insert(dataItem);
            //Entrada estoque do Vendedor           
            dataItem.tb_stock_list_id = item.tb_stock_list_id_des;
        ...
        resolve({
          result: true,
          message: 'AuditLogStockStatementOrderStockTransfer efetuada com Sucesso'
        });
      } catch (error) {
        reject("AuditLogStockStatementOrderStockTransfer: " + error);
      }
    });
    return promise;
  }
      ...
      try {
        Tb.sequelize
          .query(
            ...
            'ori.tb_stock_list_id, ord.dt_record, ori.tb_product_id, ori.quantity, ost.tb_stock_list_id_ori, ost.tb_stock_list_id_des '+
            'from tb_order_item ori      '+
            '    inner join tb_order_stock_transfer ost      '+
            '    on (ost.id =ori.tb_order_id)   '+
            '        and (ost.tb_institution_id = ori.tb_institution_id)  '+
            '        and (ost.terminal = ori.terminal)  '+
             
            '    inner join tb_order ord  '+
            '    on (ord.id =ost.id)   '+
            '      and (ord.tb_institution_id = ost.tb_institution_id)  '+
            '      and (ord.terminal = ost.terminal)  '+
        ...
        reject("AuditLogStockStatementOrderStockTransfer.getList: " + error);
      }
    });
    return promise;
  }
...
module.exports = AuditLogStockStatementOrderStockTransfer;
